"""
Verify detection_common resolves its source project outside the workspace cwd.
"""
...
def test_project_root_is_owned_by_importing_module(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-c",
            "from detection_common.config import SUBPROJECT_ROOT; print(SUBPROJECT_ROOT)",
        ],
        cwd=tmp_path,
        capture_output=True,
        text=True,
